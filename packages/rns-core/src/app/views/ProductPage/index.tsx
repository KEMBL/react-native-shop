...
import { PriceModel, ProductModel } from 'rns-packages';
import {
...
export default class ProductPage extends PureComponent<ProductPageProps, ProductPageState> {
  public state = {
        ...
        <ScrollView style={{ height: 550 }}>
          ...
          <View style={ItemPageTheme.priceContainer}>
            <StylableText style={ItemPageTheme.price}>
              {PriceUtils.makePriceString(product.price.properties, selectedVariantIndex, amount)}
            </StylableText>
          </View>
            ...
            {selectedVariantIndex !== -1 && (
              <View style={ItemPageTheme.amountContainer}>
                ...
                <View style={ItemPageTheme.amountSelectorContainer}>
                  <Button style={amountButton} title="-" onPress={this.onAmountDecrease} />
                  <StylableText style={ItemPageTheme.amountSelectorText}>{amount}</StylableText>
                  <Button style={amountButton} title="+" onPress={this.onAmountIncrease} />
                </View>
              </View>
            )}
            <DeliverySelector />
  ...
  private priceContainer = (priceModel: PriceModel): JSX.Element => {
    const buttons = priceModel.properties.map((p, index) => this.priceButton(index, PriceUtils.makeButtonTitle(p)));
